            ...
            // if (current_user_can('administrator')) {
                ...
                if (!empty($films_data)) {
                    ...
                    foreach ($films_data as $film_id => $film_data) {
                        ...
                        // Wins and nominations text
                        if (!empty($nominations)) {
                            $win_count = $film_data['win_count'];
                            $total_nominations = $film_data['total_nominations'];
                            $output .= '<div class="film-stats">';
                            if ($win_count > 0) {
                                $output .= '<span class="wins-count"><span class="wins-number">' . $win_count . '</span> Win' . ($win_count != 1 ? 's' : '') . '</span>';
                                $output .= ' / ';
                            }
                            $nom_text = ($win_count > 0) ? 'Nom' : 'Nomination';
                            $output .= '<span class="noms-count"><span class="noms-number">' . $total_nominations . '</span> ' . $nom_text . ($total_nominations != 1 ? 's' : '') . '</span>';
                            $output .= '</div>';
                            
                            // Categories details block
                            $output .= '<details class="film-categories-details">';
                            $output .= '<summary>Categories</summary>';
                            $output .= '<ul class="categories-list">';
                            foreach ($nominations as $nomination) {
                                $category_class = $nomination['is_winner'] ? 'category-winner' : 'category-nominee';
                                $category_slug = sanitize_title($nomination['category_name']);
                                $output .= '<li class="' . $category_class . '">';
                                $output .= '<a href="https://oscarschecklist.com/category-pages/' . $category_slug . '">';
                                $output .= esc_html($nomination['category_name']);
                                $output .= '</a>';
                                $output .= '</li>';
                            }
                            $output .= '</ul>';
